#include "RTK.h"
#include <process.h>
#include <windows.h>
#include "rinex.h"

#define RINEX 1
#define HTNEX 0

int iobs = 0;
int GetEpochObs(obs_t *obss, obsd_t * epochobs) //获取单历元观测量,返回观测量个数
{
	int n = 1, iobs0;
	iobs0 = iobs;

	while (fabs(obss->data[iobs].time.time - obss->data[iobs+1].time.time) < EPSILON)
	{
		iobs++;
		n++;
	}
	memcpy(epochobs, &(obss->data[iobs0]), n*sizeof(obsd_t));
	iobs++;
	return n;
}
void fillNavData(const nav_t *nav, T_NAV *navt) //星历数据转换nav_t->T_NAV
{
	UINT32 i=0;
	navt->n = nav->n;
	navt->nmax = nav->nmax;
	navt->ng = nav->ng;
	navt->ngmax = nav->ngmax;
	for(i=0;i<nav->n;i++)
	{
		navt->eph[i].sat = nav->eph[i].sat;
		navt->eph[i].iode = nav->eph[i].iode;
		navt->eph[i].iodc = nav->eph[i].iodc;
		navt->eph[i].sva = nav->eph[i].sva;
		navt->eph[i].svh = nav->eph[i].svh;
		navt->eph[i].week = nav->eph[i].week;
		navt->eph[i].code = nav->eph[i].code;
		navt->eph[i].toe = (double)nav->eph[i].toe.time+nav->eph[i].toe.sec;
		navt->eph[i].toc = (double)nav->eph[i].toc.time+nav->eph[i].toc.sec;
		navt->eph[i].ttr = (double)nav->eph[i].ttr.time+nav->eph[i].ttr.sec;
		navt->eph[i].A = nav->eph[i].A;
		navt->eph[i].e = nav->eph[i].e;
		navt->eph[i].i0 = nav->eph[i].i0;
		navt->eph[i].OMG0 = nav->eph[i].OMG0;
		navt->eph[i].omg = nav->eph[i].omg;
		navt->eph[i].M0 = nav->eph[i].M0;
		navt->eph[i].deln = nav->eph[i].deln;
		navt->eph[i].OMGd = nav->eph[i].OMGd;
		navt->eph[i].idot = nav->eph[i].idot;
		navt->eph[i].crc = nav->eph[i].crc;
		navt->eph[i].crs = nav->eph[i].crs;
		navt->eph[i].cuc = nav->eph[i].cuc;
		navt->eph[i].cus = nav->eph[i].cus;
		navt->eph[i].cic = nav->eph[i].cic;
		navt->eph[i].cis = nav->eph[i].cis;
		navt->eph[i].toes = nav->eph[i].toes;
		//navt->eph[i].fit = nav.eph[i].fit;
		navt->eph[i].f0 = nav->eph[i].f0;
		navt->eph[i].f1 = nav->eph[i].f1;
		navt->eph[i].f2 = nav->eph[i].f2;
		memcpy(navt->eph[i].tgd, nav->eph[i].tgd, sizeof(float64_t)* 6);

		
	}
	
	
}

void fillObsData(const obsd_t *obs, UINT8 n, T_OBSS *obss) //观测数据转换obsd_t->T_OBSS
{
	UINT8 i;
	UINT8 nr=0, nb=0;
	obss->n = n;
	for(i=0;i<n;i++)
	{
		obss->data[i].time = (float64_t)obs[i].time.time+ obs[i].time.sec;
		obss->data[i].sat = obs[i].sat;
		if (obs[i].rcv == 1)
			nr++;
		else
			nb++;
			
		obss->data[i].rcv = obs[i].rcv;			
		memcpy(obss->data[i].SNR, obs[i].SNR, NFREQ*sizeof(float64_t));
		memcpy(obss->data[i].code, obs[i].code, NFREQ*sizeof(float64_t));
		memcpy(obss->data[i].L, obs[i].L, NFREQ*sizeof(float64_t));
		memcpy(obss->data[i].P, obs[i].P, NFREQ*sizeof(float64_t));
		memcpy(obss->data[i].D, obs[i].D, NFREQ*sizeof(float64_t));
		
	}
	obss->nr = nr;
	obss->nb = nb;

		
}

int main()
{
	int i,nsat, nepoch;
	double ep[6] = { 0 }, rr[3] = { 0 }, pos[3] = { 0 }, enu[3] = { 0 };
	obs_t obss = {0};
	obsd_t epochobs[MAXSAT] = { 0 };
	nav_t nav = { 0 };
	int ifix = 0, isol = 0;

	T_PRCOPT opt = {0};
	T_OBSS obst = { 0 };
	T_NAV navt = { 0 };
	T_RTK rtk = {0};
	
#if RINEX
	//char *infiles[3] = { "E:\\Project\\data\\20210525\\gnss_data.obs",
	//					"E:\\Project\\data\\20210525\\B1145M.21O",
	//					"E:\\Project\\data\\20210525\\gnss_data.nav" };
	//char *outfile = { "E:\\Project\\data\\20210525\\Rinex_result.pos" };

	char *infiles[3] = { "E:\\Project\\data\\20210616\\20210616-01\\gnsicm1_00000000.obs",//移动站观测文件
		"E:\\Project\\data\\20210616\\20210616-01\\B1167E.21O", //基准站观测文件
		"E:\\Project\\data\\20210616\\BRDC00IGS_R_20211670000_01D_MN.rnx" };// 星历文件E:\\Project\\data\\20210616\\20210616-01\\gnsicm1_00000000.nav
	char *outfile = { "E:\\Project\\data\\20210616\\20210616-01\\result.pos" };//解算结果文件

	//char *infiles[3] = { "E:\\Project\\data\\20210616\\20210616-06\\gnsicm1_00000005.obs",
	//	"E:\\Project\\data\\20210616\\20210616-06\\B1167L2.21O",
	//	"E:\\Project\\data\\20210616\\BRDC00IGS_R_20211670000_01D_MN.rnx" };//E:\\Project\\data\\20210616\\20210616-01\\gnsicm1_00000000.nav
	//char *outfile = { "E:\\Project\\data\\20210616\\20210616-06\\result.pos" };

	opt.cnrmask = 32;
	opt.elmin = 15.0*D2R;
	opt.maxgdop = 30;
	opt.sigL[0][0] = 0.015;opt.sigL[0][1] = 0.015;
	opt.sigL[1][0] = 0.03;opt.sigL[1][1] = 0.03;
	opt.sigP[0][0] = 0.3;opt.sigP[0][1] = 0.3;
	opt.sigP[1][0] = 0.3;opt.sigP[1][1] = 0.3;
	opt.lambdaffrtmask = 0.5;
//	opt.ionoopt = IONOOPT_IFLC;//IONOOPT_IFLC
	opt.modear = MOD_WL;

	if ((ofp[0] = fopen(outfile, "w")) == NULL) return 0;
	ReadRinex(infiles, 3, &obss, &nav, &nepoch);

	rr[0] = Epoch2time(ep);

#elif HTNEX
	char *infiles[3] = { "E:\\Project\\data\\20210525\\ReceivedTofile-TCPCLIENT-2021_5_25_20-38-52.DAT",
		"E:\\Project\\data\\20210525\\range_data(3).txt",//"E:\\Project\\data\\20200525\\range_data(3).txt",
		"E:\\Project\\data\\20210525\\ephem_data(2).txt" };
	char *outfile = { "E:\\Project\\data\\20210525\\Htnex_result.pos" };

	if ((ifp[0] = fopen(infiles[0], "rb")) == NULL) return 0;
	if ((ifp[1] = fopen(infiles[1], "rb")) == NULL) return 0;
	if ((ifp[2] = fopen(infiles[2], "rb")) == NULL) return 0;
	if ((ofp[0] = fopen(outfile, "w")) == NULL) return 0;

	if (!NavUpdate(ifp[2], &nav))return 0;
	if (!RtcmRead(ifp[0], &obss, ep)) return 0;
	if (!ObsRead(ifp[1], &obss)) return 0;
	for (i = 0; i < obss.n; i++)
	{
		time2epoch(obss.data[i].time, ep);
		fprintf(ofp[0], "\n RTK %4d%02d%02d %02d %02d %05.2f ", (int)ep[0], (int)ep[1], (int)ep[2], (int)ep[3], (int)ep[4], ep[5]);
		fprintf(ofp[0], "%02d %d %.3f %.3f %.3f P %.3f %.3f %.3f", obss.data[i].sat, obss.data[i].rcv, obss.data[i].L[0], obss.data[i].L[1], obss.data[i].L[2],obss.data[i].P[0], obss.data[i].P[1], obss.data[i].P[2]);
	}
	
#endif

#if 1
	fillNavData(&nav, &navt); //导航数据填充
	while (1)
	{
		
		memset(&rtk, 0, sizeof(T_RTK));
		memset(&obst, 0, sizeof(T_OBSS));

		nsat = GetEpochObs(&obss, epochobs);//获取每个历元数据
		fillObsData(epochobs, nsat, &obst); //观测数据填充

		if (iobs >= obss.n) break;	//数据全部读取，退出	
		Time2epoch(obst.data[0].time, ep); //解算时刻
		printf("\rRTK-test solution:%4d%02d%02d %02d %02d %05.2f", (int)ep[0], (int)ep[1], (int)ep[2], (int)ep[3], (int)ep[4], ep[5]);
	//	if ((double)epochobs[0].time.time < 1623816300) continue;// 1621945783		
		if (obst.nr < 4 || obst.nb < 4) continue; //卫星数少，不解算
		isol++;
 		if (!RtkPos(&opt, &obst, &navt, &rtk)) continue; //解算主函数
		if(rtk.solrover.stat == SOLQ_FIX)
				ifix++;			
	}
	fclose(ofp[0]);
	printf("\n总历元数：%d\n固定历元数:%d\n固定率：%f",isol,ifix,(double)ifix/(double)isol);
	printf("\n按任意键结束");
	getchar();
#endif
	
	return 0;
}