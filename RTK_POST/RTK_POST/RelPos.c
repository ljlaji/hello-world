...
float64_t atan3(float64_t y, float64_t x) {
	float64_t t, z, z2, ret, oneIfXXZero,xx,yy,tmp;
	// move in first octant
	xx = fabs(x);
	if (xx<EPSILON) return 0;
	yy = fabs(y);
	tmp = 0;
	if (yy>xx) {
		tmp = yy;
...
float64_t atan4(float64_t y, float64_t x){
	float64_t t0, t1, t2, t3, t4;
	t3 = fabs(x);
	t1 = fabs(y);
	t0 = max(t3, t1);
	t1 = min(t3, t1);
                     ...
                     float64_t *var)
{
  ...
  //  u=atan2(sqrt(1.0-eph->e*eph->e)*sinE,cosE-eph->e)+eph->omg;
	u=atan3(sqrt(1.0-eph->e*eph->e)*sinE,cosE-eph->e)+eph->omg;
	//x = (u - r)*57.3;
	//fprintf(ofp[0],"\n%f %f %f",u,r,x);
...
extern void Ecef2Pos(const float64_t *r, float64_t *pos)
{
  ...
  //  pos[1]=r2>1E-12?atan2(r[1],r[0]):0.0;
	//if (!isnormal(pos[1]))
	//	pos[1] = r2>1E-12 ? atan2(r[1], r[0]) : 0.0;
	pos[1]=r2>1E-12?atan3(r[1],r[0]):0.0;
...
extern float64_t SatAzel(const float64_t *pos, const float64_t *e, float64_t *azel)
{
    ...
    if (pos[2]>-RE_WGS84) {
       ...
       // az=Dot(enu,enu,2)<1E-12?0.0:atan2(enu[0],enu[1]);
		az=Dot(enu,enu,2)<1E-12?0.0:atan3(enu[0],enu[1]);
        if (az<0.0) az+=2*PI;
        el=asin(enu[2]);
    }
...
extern void AdjObss(const T_PRCOPT *opt,T_OBSS *obss,T_SSAT *ssat)
{
	...
	for(i=0;i<obss->nr;i++)
	{
		for(j=0;j<NFREQ;j++)
		{
			if (obss->data[i].SNR[j]*SNR_UNIT < opt->cnrmask && fabs(obss->data[i].P[j])>EPSILON)
			{
				//DeleteObs(obss, obss->data[i].sat);
				obss->data[i].P[j] = 0;
				obss->data[i].L[j] = 0;
