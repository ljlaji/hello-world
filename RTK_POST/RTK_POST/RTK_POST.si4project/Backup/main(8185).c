...
int ObsRead(FILE *fp,obs_t *obss) //read obs file
{
	...
	while (!feof(fp))
	{
		fgets(strline, MAXSTRNUM, fp); //get each line data
		printf("\n%d", ++k);


		GetSplitString(strline, delim, &frame); //split line to string 
		/*frame_data[sat][sat_data] 1:27:
周数	周内秒	跟踪状态	通道号	相位锁定	校验已知	伪码锁定	相关器类型	卫星系统	预留	分组标志	信号类型	预留	L1为首要通道	载波相位测量值+半周	滤波器指示	PRN锁定标志	通道分配	多普勒	伪距	ADR	伪距标准差	ADR标准差	卫星号	锁定时间	载噪比		GLONASS频带数
		...
		for (i = 0; i < nsat; i++) //load date by satellites
		{
			...
			for (j = 0; j < obss->n; j++)
			{
				if (obss->data[j].sat == obs_data->sat && obss->data[j].rcv == obs_data->rcv &&fabs(timediff(obss->data[j].time, obs_data->time)) < DTTOL) //if the same satellite,different freq
				{
					...
					obss->data[j].code[idx] = code;
					obss->data[j].SNR[idx] = frame_data[i][25];
					obss->data[j].LLI[idx] = 0;
					obss->data[j].P[idx] = frame_data[i][19];
					obss->data[j].D[idx] = frame_data[i][18];
					fre = sat2freq(obs_data->sat, obs_data->code[idx], NULL);
					lamb = CLIGHT / fre;
					adr = frame_data[i][20];
					adr_roll = (obs_data->P[idx] / lamb + adr) / max_value;
					adr_roll = ROUND(adr_roll);
					obss->data[j].L[idx] = adr - max_value*adr_roll;
					break;
				}
			}
			if (j == obss->n)
