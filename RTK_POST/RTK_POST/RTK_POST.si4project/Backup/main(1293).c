...
void fillObsData(const obsd_t *obs, UINT8 n, T_OBSS *obss)
{
	...
	for(i=0;i<n;i++)
	{
		...
		memcpy(obss->data[i].LLI, obs[i].LLI, NFREQ*sizeof(float64_t));
...
{
	int i,nsat, nepoch;
	double ep[6] = { 2021, 5, 25, 12, 29, 43 }, rr[3] = { 0 }, pos[3] = { 0 }, enu[3] = { 0 };
	obs_t obss = {0};
	...
	opt.elmin = 15.0*D2R;
	opt.maxgdop = 30;
	opt.sigL[0][0] = 0.015;opt.sigL[0][1] = 0.015;opt.sigL[0][2] = 0.015;
	opt.sigL[1][0] = 0.03;opt.sigL[1][1] = 0.03;opt.sigL[1][2] = 0.03;
	opt.sigP[0][0] = 0.3;opt.sigP[0][1] = 0.3;opt.sigP[0][2] = 0.3;
	opt.sigP[1][0] = 0.3;opt.sigP[1][1] = 0.3;opt.sigP[1][2] = 0.3;
	...
	while (1)
	{
		...
		//if ((double)epochobs[0].time.time < 1621945783) continue;
		if ((double)epochobs[0].time.time < 1621946240) continue;
		isol++;
		//if(!rtkpos(&rtk, epochobs, nsat, &nav)) continue;	
		if (!PntPos(&opt, obst.data, obst.nr, &navt, &SolR, &SsatR));
			//continue;
		if (!PntPos(&opt, obst.data + obst.nr, obst.nb, &navt, &SolB, &SsatB)) continue;
		DDObsWL(&opt, &obst, &SolR, &SsatR, &SolB, &SsatB, &ddobs);
		Time2epoch(obst.data[0].time, ep);
		for (i = 0; i < COORDIM; i++)
		{
			rtk.rr[i] = SolR.rr[i];
		...
		RtkPos(&opt, &ddobs, &SsatR, &SsatB,&rtk);
		ifix++;
//		fprintf(ofp[0], "\n RTK %4d%02d%02d %02d %02d %05.2f ", (int)ep[0], (int)ep[1], (int)ep[2], (int)ep[3], (int)ep[4], ep[5]);
//		fprintf(ofp[0], "%d,%d,%.2f,%.2f,%.2f", SolR.stat, SolR.ns, SolR.rr[0], SolR.rr[1], SolR.rr[2]);
