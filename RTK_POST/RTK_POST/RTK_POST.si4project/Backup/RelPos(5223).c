               ...
               float64_t *Qx)
{
 ...
 //   MatMul("NN",m,m,n,1.0,A_Qinv,A,0.0,Qxinv);/* A'*Q^-1*A */
	MatTrans(A, n, m, At);
	MatMuls(At, Qinv, m,n,n,A_Qinv); /* A'*Q^-1 */
	MatMuls(A_Qinv, A,m,n,m,Qxinv);/* A'*Q^-1*A */

	if(MatInv(Qxinv,m,Qx)<0) return 0;
	MatMuls(Qx, A_Qinv,m,m,n,Tmp);/* (A'*Q^-1*A)^-1*A'*Q^-1 */
	MatMuls(Tmp, y,m,n,1, x);/* x=(A'*Q^-1*A)^-1*A'*Q^-1*y */
    return 1;
}	
...
extern uint8_t RtkPos(const T_PRCOPT *opt, const T_OBSS *obst, const T_NAV *navt,T_RTK *rtk)
{
	...
	if (!PntPos(opt, obst->data, obst->nr, navt, &SolR, &SsatR)); //移动站单点定位
	//	return 0;
	if (!PntPos(opt, obst->data + obst->nr, obst->nb, navt, &SolB, &SsatB)) //基准站单点定位
		return 0;
	for (i = 0; i < COORDIM; i++)
	{
		rtk->rr[i] = SolR.rr[i];
	...
	if (0)
	{
		DDObs(opt, obst, &SolR, &SsatR, &SolB, &SsatB, &ddobs); //双差观测量获取
		WeightObs(&ddobs, opt->sigP, opt->sigL, SsatR.ele, SsatB.ele, ddQP, ddQL); //观测量噪声方差阵	
	}
	else   
	{
		DDObsWL(opt, obst, &SolR, &SsatR, &SolB, &SsatB, &ddobs); //宽巷双差观测量获取
	...
	for (i = 0; i<COORDIM; i++)
	{
			bfix[i] = rtk->rr[i] - rtk->rb[i] + bfix[i];
