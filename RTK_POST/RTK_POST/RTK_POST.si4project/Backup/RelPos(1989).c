...
extern uint8_t RtkPos(const T_PRCOPT *opt, const T_OBSS *obst, const T_NAV *navt,T_RTK *rtk)
{
	...
	if (opt->mode == MOD_WL)
	{
		DDObsWL(opt, obst, &rtk->solrover, &SsatR, &rtk->solbase, &SsatB, &ddobs); //宽巷双差观测量获取
		WeightObsWL(&ddobs, opt->sigP, opt->sigL, SsatR.ele, SsatB.ele, ddQP, ddQL); //宽巷观测量噪声方差阵	
	}
	else 
	{

		DDObs(opt, obst, &rtk->solrover, &SsatR, &rtk->solbase, &SsatB, &ddobs); //双差观测量获取
	...
	fprintf(ofp[0], "\r\n%f %d %d %f %f %f %f %f %f", ddobs.time, rtk->solrover.stat, ddobs.num, mu[1], mu[0], bfix[0], bfix[1], bfix[2], rtk->baselinelen);
