...
*            refer setseleph()
*-----------------------------------------------------------------------------*/
extern sint16_t GetSelEph(sint16_t sys)
{
    switch (sys) {
...
*          (tgd or bgd)
*-----------------------------------------------------------------------------*/
extern void Eph2Pos(const float64_t time, const T_EPH *eph,  float64_t *rs,  float64_t *dts,
                     float64_t *var)
{
...
* notes  : see ref [2]
*-----------------------------------------------------------------------------*/
extern void Geph2Pos(float64_t time, const T_GEPH *geph, float64_t *rs, float64_t *dts,
                     float64_t *var)
{
    float64_t t,tt,x[6];
    uint8_t  i;
...
{
    float64_t e2=FE_WGS84*(2.0-FE_WGS84),r2=Dot(r,r,2),z,zk,v=RE_WGS84,sinp;
...
extern void MatMul(const uint8_t *tr, uint16_t n, uint16_t k, uint16_t m, float64_t alpha,
                   const float64_t *A, const float64_t *B, float64_t beta, float64_t *C)
{
    float64_t d;
    uint16_t i,j,x,f=tr[0]=='N'?(tr[1]=='N'?1:2):(tr[1]=='N'?3:4);
...
void MatMuls(const float64_t A[],const float64_t B[],uint16_t iM,uint16_t iN,uint16_t iK,float64_t C[])
{
	uint16_t i,j,t;
...
/*矩阵求逆 A=B(-1)*/
sint32_t MatInv(const float64_t A[],uint32_t m,float64_t B[])
{
...
/*单位矩阵*/
void Eye(float64_t *A,uint32_t iRow)
{
	uint32_t i;
...
/*对角矩阵*/
void Diag(const float64_t A[],uint32_t iRow,float64_t B[])
{
	uint32_t i;
...
/*矩阵转置 B=A'*/
void MatTrans(const float64_t A[],uint32_t iRow,uint32_t iCol,float64_t B[])
{
	uint32_t i,j;
...
void Zeros(float64_t *A,uint16_t n,uint16_t m)
{
	memset(A,0,n*m*sizeof(float64_t));
}
/*矩阵相加 C=A+B*/
void AddMat(const float64_t *A,const float64_t *B,uint16_t iRow,uint16_t iCol,float64_t *C)
{
	uint16_t i,j;
...
/*矩阵相减 C=A-B*/
void SubMat(const float64_t *A,const float64_t *B,uint16_t iRow,uint16_t iCol,float64_t *C)
{
	uint16_t i,j;
...
* return : none
*-----------------------------------------------------------------------------*/
extern void MatCpy(double *A, const float64_t *B, uint32_t n, uint32_t m)
{
    memcpy(A,B,sizeof(float64_t)*n*m);
}

/* combine matrix -----------------------------------------------------------------
* args   : int flag         O	combine style,0:C=[A,B], 1:C=[A
															  B]
*		   double *A        O   matrix A (n x m)
*		   double *B        O   matrix B (p x q)
*          int    n,m,p,q   O   number of rows and columns of matrix A,B(0:n=p,1:m=q)
*		   double *C        O   destination matrix C (0:n x (m+q),1:(n+p)xm)

* return : 0:error, 1:normal
*-----------------------------------------------------------------------------*/

extern void MatCombine(uint8_t flag,const float64_t *A,const uint16_t n,const uint16_t m,const float64_t *B,const uint16_t p,const uint16_t q,float64_t *C)
{
	uint16_t i,j;
...
/*
get matrix  B[p][q] from matrix A[n][m],start from row0,col0; 
*/
extern void MatSelect(const float64_t *A,uint16_t n,uint16_t m, uint16_t row0,uint16_t col0,uint16_t p,uint16_t q,float64_t *B)
{
	uint16_t i,j;
	for(j=0;j<q;j++)
	{
...
{
    int info;
	float64_t Qinv[MAXDIM*MAXDIM]={0},A_Qinv[MAXDIM*MAXDIM]={0},Qxinv[MAXDIM*MAXDIM]={0},Tmp[MAXDIM*MAXDIM];
...
{
	float64_t H[4 * MAXSAT] = { 0 }, Q[16] = { 0 }, Qinv[16] = { 0 }, cosel = { 0 }, sinel = {0};
    uint8_t i,n;
...
extern uint8_t PntPos(const T_PRCOPT *opt,const T_OBS *obs, const uint8_t n, const T_NAV *nav,T_SOL *sol,T_SSAT  *ssat)
{
	...
	if(ssat) /*save sat data*/
	{
		...
		//if (n==25)
		//{
		//	sol->rr[0] = -2328330.1373431762;
		//	sol->rr[1] = 5385387.9619346876;
		//	sol->rr[2] = 2493085.0181376431;
		//}
		//else
		//{
		//	sol->rr[0] = -2328330.7154120905;
		//	sol->rr[1] = 5385389.7314946931;
		//	sol->rr[2] = 2493084.9324697070;
		//}
		...
		for(i=0;i<n;i++)
		{
			isat = obs[i].sat;
			ssat->sat[isat] = isat;
			memcpy(&ssat->pos[isat],rs+i*6,COORDIM*sizeof(float64_t));
			memcpy(&ssat->vel[isat],rs+i*6+3,COORDIM*sizeof(float64_t));
			ssat->ele[isat] = azel[i*2+1];
			ssat->azi[isat] = azel[i*2];
			
		//	for(j=0;j<COORDIM;j++) los[j] = rs[i*6+j]-sol->rr[j];
			//dis =  Norm(los,COORDIM);
			//for(j=0;j<COORDIM;j++) los[j] = los[j]/dis;
			dis = GeoDist(rs + i * 6, sol->rr, los);
			
			memcpy(&ssat->los[isat],los,COORDIM*sizeof(float64_t));
			ssat->dis[isat] = dis;			
		}
		ssat->n = n;
	}
...
//get system and frequent according to double-differenced index
static uint8_t idd2SysFre(uint8_t idd,uint8_t ddnum[NUM_SYS][NUM_FRE],uint8_t *isf)
{
	uint8_t i,j,n=0;
	for(i=0;i<NUM_SYS;i++)
		for(j=0;j<NUM_FRE;j++)
...
float64_t g_wavelen[2][3] = {{0.190293672798365,0.244210213424568,0.254828048790854},{0.192039486310276,0.248349369584307,0.236332464604421}};
float64_t g_coefWL[3] = { 41.031141868512115, 38.903337899695956, 31.184746877054572 };//2*waveWL*waveWL/waveL1/waveL1
		...
		for(j=0;j<NUM_FRE;j++)
		{
			...
			for(k=0;k<obss->nr;k++)
			{
				...
				if (ssatB->ele[sat]>minele[i][j])
				{
					...
					refSDP[i][j] = obss->data[k].P[j] - obss->data[k+obss->nr].P[j];
...
extern void DDObsWL(const T_PRCOPT *opt,const T_OBSS *obss,const T_SOL *solR,const T_SSAT  *ssatR,const T_SOL *solB,const T_SSAT  *ssatB,T_DDOBS *ddobs)
{
	...
	{			
		for(k=0;k<obss->nr;k++)
		{
			...
			if (ssatB->ele[sat]>minele[i])
			{
				...
				if(i==0)
					refSDP[i] = obss->data[k].P[0] - obss->data[k+obss->nr].P[0];
				else
					refSDP[i] = obss->data[k].P[1] - obss->data[k+obss->nr].P[1];
	...
	{	
		for(k=0;k<obss->nr;k++)
		{
			...
			if (i==0)
				ddobs->triddP[n] = obss->data[k].P[0] - obss->data[k + obss->nr].P[0] - refSDP[i];
			else
				ddobs->triddP[n] = obss->data[k].P[1] - obss->data[k + obss->nr].P[1] - refSDP[i];
...
extern void WeightObs(const T_PRCOPT *opt,const T_SSAT  *ssatR,const T_SSAT  *ssatB,T_DDOBS *ddobs,float64_t *ddQL,float64_t *ddQP)
{
	...
	for(i=0;i<NUM_SYS;i++)
		for(j=0;j<NUM_FRE;j++)
		{
			...
			{
				varRL = VarEle(ssatR->ele[sat], opt->sigL[i][j] * g_wavelen[i][j]);
				varBL = VarEle(ssatB->ele[sat], opt->sigL[i][j] * g_wavelen[i][j]);
				varRP = VarEle(ssatR->ele[sat], opt->sigP[i][j]);
				varBP = VarEle(ssatB->ele[sat], opt->sigP[i][j]);
			}			
	...
	for (i = 0; i<ddobs->num; i++)
	{
		...
		sat = ddobs->ddsat[i];
		varRL = VarEle(ssatR->ele[sat], opt->sigL[isf[0]][isf[1]] * g_wavelen[isf[0]][isf[1]]);
		varBL = VarEle(ssatB->ele[sat], opt->sigL[isf[0]][isf[1]] * g_wavelen[isf[0]][isf[1]]);
		varRP = VarEle(ssatR->ele[sat], opt->sigP[isf[0]][isf[1]]);
		varBP = VarEle(ssatB->ele[sat], opt->sigP[isf[0]][isf[1]]);
	...
	MatTrans(ddT, ddobs->num, n, ddTt);
	MatMuls(ddT, sdvarLmat, ddobs->num, n, n, Tmp);
	MatMuls(Tmp, ddTt, ddobs->num, n, ddobs->num, ddQL);
	
	MatMuls(ddT, sdvarPmat, ddobs->num, n, n, Tmp);
	MatMuls(Tmp, ddTt, ddobs->num, n, ddobs->num, ddQP);
				
}
extern void WeightObsWL(const T_PRCOPT *opt, const T_SSAT  *ssatR, const T_SSAT  *ssatB, T_DDOBS *ddobs, float64_t *ddQL, float64_t *ddQP)
{
	uint8_t i, j, k, n = 0, n0 = 0;
	uint8_t sat, isf[3] = { 0 };
	float64_t ddT[MAXOBS*NUM_FRE*MAXOBS*NUM_FRE] = { 0 }, ddTt[MAXOBS*NUM_FRE*MAXOBS*NUM_FRE] = { 0 };
	...
	for (i = 0; i<NUM_SYS; i++)
	{
		...
		{
			varRL = VarEle(ssatR->ele[sat], opt->sigL[i][0] * g_wavelen[i][0] );
			varBL = VarEle(ssatB->ele[sat], opt->sigL[i][0] * g_wavelen[i][0] );
			varRP = VarEle(ssatR->ele[sat], opt->sigP[i][0]);
			varBP = VarEle(ssatB->ele[sat], opt->sigP[i][0]);
		}
	...
	for (i = 0; i<ddobs->num; i++)
	{
		...
		varRP = VarEle(ssatR->ele[sat], opt->sigP[isf[0]][isf[1]]);
		varBP = VarEle(ssatB->ele[sat], opt->sigP[isf[0]][isf[1]]);
		sdvarL[n] = (varRL + varBL)*g_coefWL[isf[0]];
		sdvarP[n] = varRP + varBP;
...
/*calculate the fixed baseline according to float solution 
bfix = bfloat-Qba*Qaf^-1*(afloat-afix)
*/
void FixBaseline(uint8_t n,const float64_t * bfloat,const float64_t*afloat, const float64_t *afix, const float64_t * Qaf, const float64_t * Qbaf,float64_t *bfix)
{
	float64_t da[MAXOBS*NUM_FRE]={0},iQaf[MAXOBS*NUM_FRE*MAXOBS*NUM_FRE]={0},Tmp[COORDIM*MAXOBS*NUM_FRE]={0},db[COORDIM]={0};
	uint8_t i;
	MatInv(Qaf, n, iQaf);
	MatMuls(Qbaf, iQaf, COORDIM, n, n, Tmp);
	SubMat(afloat, afix, n, 1, da);
	MatMuls(Tmp, da, COORDIM, n, 1, db);
	SubMat(bfloat,db,COORDIM,1,bfix);			
}
extern void RtkPos(const T_PRCOPT *opt, const T_DDOBS *ddobs, const T_SSAT  *ssatR, const T_SSAT  *ssatB,T_RTK *rtk )
{

	uint8_t i=0,j=0,k=0,n=0,n0=0;
	...
	float64_t Q[MAXOBS*NUM_FRE*2*MAXOBS*NUM_FRE*2],Qx[(COORDIM+MAXOBS*NUM_FRE)*(COORDIM+MAXOBS*NUM_FRE)],ddQL[MAXOBS*NUM_FRE*MAXOBS*NUM_FRE],ddQP[MAXOBS*NUM_FRE*MAXOBS*NUM_FRE];
	float64_t baseline_float[COORDIM],amb_float[MAXOBS*NUM_FRE],baseline_fix[COORDIM],amb_fix[MAXOBS*NUM_FRE];
	float64_t Qbf[COORDIM*COORDIM],Qaf[MAXOBS*NUM_FRE*MAXOBS*NUM_FRE],Qbaf[COORDIM*MAXOBS*NUM_FRE],Qaft[MAXOBS*NUM_FRE*MAXOBS*NUM_FRE];
	float64_t mu[2]; 
	float64_t Pf=0.01; 
	float64_t Sqnorm[AMBSOLNUM];
	float64_t dRatio;
	sint8_t stat,flag;

	MatCombine(1,ddobs->ddP,ddobs->num,1,ddobs->ddL,ddobs->num,1,y);//y=[ddP;ddL]	
	Zeros(O, ddobs->num, ddobs->num);
	Diag(ddobs->wavelen,ddobs->num,D);
	MatTrans(ddobs->ddmat, COORDIM, ddobs->num, B);
	MatCombine(1, B, ddobs->num, COORDIM, B, ddobs->num, COORDIM, tmp1);
	MatCombine(1,O,ddobs->num,ddobs->num,D,ddobs->num,ddobs->num,tmp2);
	MatCombine(0, tmp1, ddobs->num * 2, COORDIM, tmp2, ddobs->num * 2, ddobs->num, A);//A = [H,o;H,D]
	//WeightObsWL(opt,ssatR,ssatB,ddobs,ddQL,ddQP);
	WeightObs(opt, ssatR, ssatB, ddobs, ddQL, ddQP);
	BlkDiag(ddQP,ddobs->num,ddobs->num,ddQL,ddobs->num,ddobs->num,Q);
	Lsq(A,y,Q, ddobs->num*2,ddobs->num+COORDIM, x,Qx);//y=Ax
	MatSelect(x, ddobs->num + COORDIM, 1, 0, 0, COORDIM, 1, baseline_float);//baseline_float = x[1:3]
	MatSelect(x, ddobs->num + COORDIM, 1,COORDIM, 0, ddobs->num, 1, amb_float);//amb_float = x[3:end]
	MatSelect(Qx, ddobs->num + COORDIM, ddobs->num + COORDIM, 0, 0, COORDIM, COORDIM, Qbf);
	MatSelect(Qx, ddobs->num + COORDIM, ddobs->num + COORDIM, COORDIM, COORDIM, ddobs->num, ddobs->num, Qaf);
	MatSelect(Qx, ddobs->num + COORDIM, ddobs->num + COORDIM, 0, COORDIM, COORDIM, ddobs->num, Qbaf);

	//PrintMat(x, ddobs->num + COORDIM, 1);
	//PrintMat(baseline_float, COORDIM, 1);
	//PrintMat(amb_float, ddobs->num, 1);

	//PrintMat(Qx, ddobs->num + COORDIM, ddobs->num + COORDIM);
	//PrintMat(Qbf, COORDIM, COORDIM);
	//PrintMat(Qaf, ddobs->num, ddobs->num);	
	//PrintMat(Qbaf, COORDIM, ddobs->num);
	////PrintMat(amb_float, ddobs->num, 1);
	//PrintMat(Qaf, ddobs->num, ddobs->num);
	//stat = lambda(ddobs->num, AMBSOLNUM, amb_float, Qaf, amb_fix, Sqnorm);
	stat = LambdaFFRT(ddobs->num, AMBSOLNUM, amb_float, Qaf, Pf,amb_fix, Sqnorm, mu);
	if (!stat)
	{
		FixBaseline(ddobs->num, baseline_float, amb_float, amb_fix, Qaf, Qbaf, baseline_fix);
		for (i = 0; i<COORDIM; i++)
		{
			baseline_fix[i] = rtk->rr[i] - rtk->rb[i] + baseline_fix[i];
			rtk->baseline[i] = baseline_fix[i];
			rtk->rr[i] = rtk->rb[i] + baseline_fix[i];
		}
		...
		if ((mu[1]-mu[0])>0.5)
			...
			rtk->stat = 5;
		fprintf(ofp[0], "\r\n%f %d %d %f %f %f %f %f %f", ddobs->time, rtk->stat, ddobs->num, mu[1], mu[0], baseline_fix[0], baseline_fix[1], baseline_fix[2], rtk->baselinelen);
	}
