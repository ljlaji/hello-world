...
uint8_t CalFloatSol(const T_DDOBS *ddobs,const float64_t *ddQP,const float64_t *ddQL,float64_t *bfloat,float64_t *afloat,float64_t *Qbfloat,float64_t *Qafloat,float64_t *Qbafloat)
{
	...
	MatSelect(x, ddobs->num + COORDIM, 1, 0, 0, COORDIM, 1, bfloat);//bfloat = x[1:3]
	MatSelect(x, ddobs->num + COORDIM, 1,COORDIM, 0, ddobs->num, 1, afloat);//afloat = x[3:end]
	MatSelect(Qx, ddobs->num + COORDIM, ddobs->num + COORDIM, 0, 0, COORDIM, COORDIM, Qbfloat);
	MatSelect(Qx, ddobs->num + COORDIM, ddobs->num + COORDIM, COORDIM, COORDIM, ddobs->num, ddobs->num, Qafloat);
	MatSelect(Qx, ddobs->num + COORDIM, ddobs->num + COORDIM, 0, COORDIM, COORDIM, ddobs->num, Qbafloat);
	return 1;
}
...
void FixBaseline(uint8_t n,const float64_t * bfloat,const float64_t*afloat, const float64_t *afix, const float64_t * Qafloat, const float64_t * Qbafloat,float64_t *bfix)
{
	...
	uint8_t i;
	MatInv(Qafloat, n, iQaf);
	MatMuls(Qbafloat, iQaf, COORDIM, n, n, Tmp);
	SubMat(afloat, afix, n, 1, da);
	MatMuls(Tmp, da, COORDIM, n, 1, db);
	SubMat(bfloat,db,COORDIM,1,bfix);			
}
extern uint8_t RtkPos(const T_PRCOPT *opt, const T_DDOBS *ddobs, const T_SSAT  *ssatR, const T_SSAT  *ssatB,T_RTK *rtk )
{

	uint8_t i=0,j=0,k=0,n=0,n0=0;
	float64_t ddQL[MAXOBS*NUM_FRE*MAXOBS*NUM_FRE],ddQP[MAXOBS*NUM_FRE*MAXOBS*NUM_FRE];
	float64_t bfloat[COORDIM],afloat[MAXOBS*NUM_FRE],bfix[COORDIM],afix[MAXOBS*NUM_FRE];
	float64_t Qbfloat[COORDIM*COORDIM],Qafloat[MAXOBS*NUM_FRE*MAXOBS*NUM_FRE],Qbafloat[COORDIM*MAXOBS*NUM_FRE];
	float64_t mu[2]; 
	float64_t Pf=0.01; 
	...
	WeightObs(ddobs,opt->sigP, opt->sigL,ssatR->ele, ssatB->ele, ddQP, ddQL); //求观测量噪声方差阵
	
	if(!CalFloatSol(ddobs,ddQP,ddQL,bfloat,afloat,Qbfloat,Qafloat,Qbafloat));//求浮点解
		return 0;
	
	if(LambdaFFRT(ddobs->num, AMBSOLNUM, afloat, Qafloat, Pf,afix, Sqnorm, mu)); //整周模糊度固定
		return 0;

	FixBaseline(ddobs->num, bfloat, afloat, afix, Qafloat, Qbafloat, bfix);//求固定解
	
	for (i = 0; i<COORDIM; i++)
	{
			bfix[i] = rtk->rr[i] - rtk->rb[i] + baseline_fix[i];
			rtk->baseline[i] = bfix[i];
			rtk->rr[i] = rtk->rb[i] + bfix[i];
	}
	...
	if ((mu[1]-mu[0])>0.5)
		...
		rtk->stat = 5;
	fprintf(ofp[0], "\r\n%f %d %d %f %f %f %f %f %f", ddobs->time, rtk->stat, ddobs->num, mu[1], mu[0], bfix[0], bfix[1], bfix[2], rtk->baselinelen);
