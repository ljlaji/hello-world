                   ...
                   FP64 *resp, UINT8 *ns)
{
    ...
    for (i=*ns=0;i<n&&i<MAXOBS;i++) {
        ...
        if (iter>0) {
            ...
            /* test SNR mask */
            if(obs[i].SNR[0]*SNR_UNIT<opt->cnrmin) continue;
			if(opt->ionoopt==IONOOPT_IFLC && obs[i].SNR[1]*SNR_UNIT<opt->cnrmin) continue;			
...
{
    FP64 H[4*MAXSAT],Q[16],cosel,sinel;
    UINT8 i,n;
