	...
	for(i=0;i<obss->n;i++)
	{
		if(obss->data[i].sat==sat)
		{
			if(obss->data[i].rcv==1)  
				...
				obss->nb--;
			memcpy(&obss->data[i],&obss->data[i+1],sizeof(T_OBS)*(n-i-1));
			memset(&obss->data[n-1],0,sizeof(T_OBS));
			obss->n = obss->n-1;			
		}
	}
}
extern void AdjObss(const T_PRCOPT *opt,T_OBSS *obss,T_SSAT *ssat)
{
	...
	for(i=0;i<obss->nr;i++) //start from rover station
	{
		...
		if(j==obss->n)	DeleteObs(obss,obss->data[i].sat);			
	}
...
extern uint8_t DDObs(const T_PRCOPT *opt,const T_OBSS *obss,const T_SOL *solR,const T_SSAT  *ssatR,const T_SOL *solB,const T_SSAT  *ssatB,T_DDOBS *ddobs)
{
	uint8_t i=0,j=0,k=0;
	uint8_t comsatnum,comsat[MAXOBS];
	uint16_t sys;
	uint8_t ddnum[NUM_SYS][NUM_FRE] = {0};
	uint8_t minele = 0;
	...
	for(i=0;i<NUM_FRE;i++)
	{
		for(j=0;j<obss->nr;j++)
		{
			sys = SatSys(obss->data[j].sat,NULL);
			if (fabs(obss->data[j].P[i]) < EPSILON || fabs(obss->data[j + obss->nr].P[i]) < EPSILON
				|| fabs(obss->data[j].L[i]) < EPSILON || fabs(obss->data[j + obss->nr].L[i]) < EPSILON)
				continue;
			if (ssatB)
