...
extern void RtkPos(const T_PRCOPT *opt, const T_DDOBS *ddobs, const T_SSAT  *ssatR, const T_SSAT  *ssatB)
{
	...
	float64_t Qbf[COORDIM*COORDIM],Qaf[MAXOBS*NUM_FRE*MAXOBS*NUM_FRE],Qbaf[COORDIM*MAXOBS*NUM_FRE];
	float64_t mu[2]; 
	float64_t Pf=0.01; 
	...
	PrintMat(A, ddobs->num * 2, ddobs->num + COORDIM);
	PrintMat(Q, ddobs->num * 2, ddobs->num * 2);
