...
static UINT8 EstPos(const T_OBS *obs, const UINT8 n, const FP64 *rs, const FP64 *dts,
                  ...
                  T_SOL *sol, FP64 *azel, UINT8 *vsat,FP64 *resp)
{
...
* return : status(1:ok,0:error)
*-----------------------------------------------------------------------------*/
extern UINT8 PntPos(const T_PRCOPT *opt,const T_OBS *obs, const UINT8 n, const T_NAV *nav,T_SOL *sol,T_SSAT  *ssat)
{
	...
	if(ssat) /*save sat data*/
	{
		for(i=0;i<n;i++)
		{
			...
			memcpy(ssat->pos[i*3],rs[i*6],3*sizeof(FP64));
			memcpy(ssat->vel[i*3],rs[i*6+3],3*sizeof(FP64));
