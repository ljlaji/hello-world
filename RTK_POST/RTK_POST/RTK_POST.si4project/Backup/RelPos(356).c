...
{
	uint8_t i=0,j=0,k=0,ii,n = 0;
	uint8_t comsatnum,comsat[MAXOBS];
	uint16_t sat,sys;
	...
	for(i=0;i<NUM_SYS;i++)		
	{
		...
		{	
			for(k=0;k<obss->nr;k++)
			{
				...
				ddobs->ddL[n] = g_wavelen[i][j]*(ddobs->triddL[n] - dddis);
...
void FixBaseline(uint8_t n,const float64_t * bfloat,const float64_t*afloat, const float64_t *afix, const float64_t * Qaf, const float64_t * Qbaf,float64_t *bfix)
{
	...
	uint8_t i;
	for(i=0;i<n;i++) da[i] = afloat[i]-afix[i];
	MatInv(Qaf, n, iQaf);
...
extern void RtkPos(const T_PRCOPT *opt, const T_DDOBS *ddobs, const T_SSAT  *ssatR, const T_SSAT  *ssatB)
{
	...
	float64_t O[MAXOBS*NUM_FRE*MAXOBS*NUM_FRE],D[MAXOBS*NUM_FRE*MAXOBS*NUM_FRE],tmp1[MAXOBS*NUM_FRE*2*COORDIM],tmp2[MAXOBS*NUM_FRE*2*MAXOBS*NUM_FRE];
	...
	MatCombine(1,ddobs->ddmat,ddobs->num,COORDIM,ddobs->ddmat,ddobs->num,COORDIM,tmp1);
	...
	//stat = Lambda_ffrt(ddobs->num, AMBSOLNUM, amb_float, Qaf, amb_fix, Sqnorm, mu, Pf);
	if (!stat)
	{
		FixBaseline(ddobs->num, baseline_float, amb_float, amb_fix, Qaf, Qbaf, baseline_fix);
		fprintf(ofp[0], "\r\n%d %.2f %.2f %.2f %.2f", stat, Sqnorm[1]/Sqnorm[0], baseline_fix[0], baseline_fix[1], baseline_fix[2]);
	}
