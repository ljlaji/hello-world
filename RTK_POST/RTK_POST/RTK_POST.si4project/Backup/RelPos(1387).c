...
*-----------------------------------------------------------------------------*/
sint8_t lambda_ffrt(uint8_t n, uint8_t m,float64_t a*, const double Q*, double F*, double s*, double mu*, double Pf)
{
	int info;
	double L[MAXDIM*MAXDIM],D[MAXDIM];
	double Z[MAXDIM*MAXDIM],z[MAXDIM];
	double E[MAXDIM*AMBSOLNUM],Et[MAXDIM*AMBSOLNUM];
	double Zt[MAXDIM*MAXDIM],Ft[MAXDIM*AMBSOLNUM];
	double ps=0;

	if (n<=0||m<=0) return -1;

	memset(&L[0],0,MAXDIM*MAXDIM*sizeof(double));
	memset(&z[0],0,MAXDIM*sizeof(double));
	memset(&Z[0],0,MAXDIM*MAXDIM*sizeof(double));
	Eye(Z,n);
	...
	if (!(info=LD(n,Q,L,D))) 
	{
		...
		if (!(info=search(n,m,L,D,z,E,s))) //LAMBDA搜索模糊度固定值
		{
			...
			if (!(info=MatInv(Z,n,Zt))) 
			{
				MatTrans(E,m,n,Et);
				MatMul(Zt,Et,n,n,m,Ft);
				MatTrans(Ft,n,m,F);
			}
		}
	}
...
extern void RtkPos(const T_PRCOPT opt,T_DDOBS ddobs,const T_SSAT  *ssatR,const T_SSAT  *ssatB)
{
	...
	float64_t Qbf[COORDIM*COORDIM],Qaf[MAXOBS*NUM_FRE*MAXOBS*NUM_FRE];
