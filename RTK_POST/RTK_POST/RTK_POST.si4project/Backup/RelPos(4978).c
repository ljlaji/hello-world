	...
	for(i=0;i<NUM_SYS;i++)		
	{
		...
		{	
			for(k=0;k<obss->nr;k++)
			{
				...
				for (ii = 0; ii < COORDIM; ii++) ddmat[ii] = ssatB->los[ddobs->refsat[i][j]*COORDIM+ii] - ssatB->los[sat*COORDIM+ii];
...
extern void WeightObs(const T_PRCOPT opt,const T_SSAT  *ssatR,const T_SSAT  *ssatB,T_DDOBS ddobs,float64_t *ddQL,float64_t *ddQP)
{
	...
	uint8_t sat,isf[3]={0};
	uint8_t ddT[MAXOBS*NUM_FRE*MAXOBS*NUM_FRE]={0},ddTt[MAXOBS*NUM_FRE*MAXOBS*NUM_FRE]={0};
	...
	for(i=0;i<ddobs.num;i++)
	{
		...
		ddT[i+(ddobs.num)*isf[2]] = -1;
		ddT[i+(ddobs.num)*(i+n0)] = 1;		
	}
...
extern void RtkPos(const T_PRCOPT opt,T_DDOBS ddobs,const T_SSAT  *ssatR,const T_SSAT  *ssatB)
{
	uint8_t i=0,j=0,k=0,n=0,n0=0;
	float64_t y[MAXOBS*NUM_FRE*2],x[COORDIM+MAXOBS*NUM_FRE],A[MAXOBS*NUM_FRE*2*(COORDIM+MAXOBS*NUM_FRE)];
	...
	MatCombine(0, tmp1, ddobs.num*2, COORDIM, D, ddobs.num*2, ddobs.num, A);//A = [H,o;H,D]
	...
	MatSelect(x, ddobs.num + COORDIM, COORDIM, 0, 0, ddobs.num, 1, amb_float);//amb_float = x[3:end]
	...
	MatSelect(Qx, ddobs.num + COORDIM, ddobs.num + COORDIM,0, 0, COORDIM, ddobs.num, Qbaf);
