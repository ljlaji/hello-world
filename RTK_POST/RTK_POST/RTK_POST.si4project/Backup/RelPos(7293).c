...
extern void MatSelect(const float64_t *A,uint16_t n,uint16_t m, uint16_t row0,uint16_t col0,uint16_t p,uint16_t q,float64_t *B)
{
	...
	for(j=0;j<q;j++)
	{
		memcpy(&B[j*p],&A[(j+col0)*n],sizeof(float64_t)*p);
	}
}
...
extern void WeightObs(const T_PRCOPT opt,const T_SSAT  *ssatR,const T_SSAT  *ssatB,T_DDOBS ddobs,float64_t *ddQL,float64_t *ddQP)
{
	...
	for(i=0;i<ddobs.ddn;i++)
	{
		...
		ddT[i+(ddobs.ddn)*isf[2]] = -1;
		ddT[i+(ddobs.ddn)*(i+n0)] = 1;		
	}
	MatTrans(ddT, ddobs.ddn, n, ddTt);
	MatMuls(ddT,sdvarLmat,ddobs.ddn,n,n,Tmp);
	MatMuls(Tmp,ddTt,ddobs.ddn,n,ddobs.ddn,ddQL);
	
	MatMuls(ddT,sdvarPmat,ddobs.ddn,n,n,Tmp);
	MatMuls(Tmp,ddTt,ddobs.ddn,n,ddobs.ddn,ddQP);
...
extern void RtkPos(const T_PRCOPT opt,T_DDOBS ddobs,const T_SSAT  *ssatR,const T_SSAT  *ssatB)
{
	...
	MatCombine(1,ddobs.ddP,ddobs.ddn,1,ddobs.ddL,ddobs.ddn,1,y);//y=[ddP;ddL]
	Zeros(O, ddobs.ddn, ddobs.ddn);
	Diag(ddobs.wavelen,ddobs.ddn,D);
	MatCombine(1,ddobs.ddmat,ddobs.ddn,COORDIM,ddobs.ddmat,ddobs.ddn,COORDIM,tmp1);
	MatCombine(1,O,ddobs.ddn,ddobs.ddn,D,ddobs.ddn,ddobs.ddn,tmp2);
	MatCombine(0, tmp1, ddobs.ddn*2, COORDIM, D, ddobs.ddn*2, ddobs.ddn, A);
	WeightObs(opt,ssatR,ssatB,ddobs,ddQL,ddQP);
	BlkDiag(ddQP,ddobs.ddn,ddobs.ddn,ddQL,ddobs.ddn,ddobs.ddn,Q);
	Lsq(A,y,Q, ddobs.ddn*2,ddobs.ddn+COORDIM, x,Qx);
