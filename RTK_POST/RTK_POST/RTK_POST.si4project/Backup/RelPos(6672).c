...
void FixBaseline(uint8_t n,const float64_t * bfloat,const float64_t*afloat, const float64_t *afix, const float64_t * Qaf, const float64_t * Qba,float64_t *bfix)
{
	float64_t da[MAXOBS*NUM_FRE],iQaf[MAXOBS*NUM_FRE*MAXOBS*NUM_FRE],Tmp[COORDIM*MAXOBS*NUM_FRE];
	uint8_t i;
	for(i=0;i<n;i++) da[i] = afloat[i]-afix[i];
	MatInv(Qaf, n, iQaf);
	MatMuls(Qba, iQaf, COORDIM, n, n, Tmp);
...
extern void RtkPos(const T_PRCOPT opt,T_DDOBS ddobs,const T_SSAT  *ssatR,const T_SSAT  *ssatB)
{
	...
	float64_t Qbf[COORDIM*COORDIM],Qaf[MAXOBS*NUM_FRE*MAXOBS*NUM_FRE],Qba[COORDIM*MAXOBS*NUM_FRE];
	float64_t mu[2]; 
	float64_t Pf=0.01; 
	...
	MatCombine(0, tmp1, ddobs.num*2, COORDIM, D, ddobs.num*2, ddobs.num, A);
	WeightObs(opt,ssatR,ssatB,ddobs,ddQL,ddQP);
	BlkDiag(ddQP,ddobs.num,ddobs.num,ddQL,ddobs.num,ddobs.num,Q);
	Lsq(A,y,Q, ddobs.num*2,ddobs.num+COORDIM, x,Qx);
	MatSelect(x, ddobs.num + COORDIM, 1, 0, 0, COORDIM, 1, baseline_float);
	MatSelect(x, ddobs.num + COORDIM, COORDIM, 0, 0, ddobs.num, 1, amb_float);
	...
	MatSelect(Qx, ddobs.num + COORDIM, ddobs.num + COORDIM,0, 0, COORDIM, ddobs.num, Qba);
	Lambda_ffrt(ddobs.num, AMBSOLNUM,amb_float, Qaf, amb_fix, Sqnorm, mu, &Pf);
