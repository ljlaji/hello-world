...
const solopt_t solopt_default = { /* defaults solution output options */
	SOLF_ENU, TIMES_GPST, 1, 3,    /* posf,times,timef,timeu */
...
static char codepris[7][MAXFREQ][16] = {  /* code priority for each freq-index */
	/*    0         1          2          3         4         5     */
	{ "CPYWMNSL", "DPYWCMNLSX", "IQX", "", "", "", "" }, /* GPS PYWCMNDLSX*/
	{ "CPABX", "PCABX", "IQX", "", "", "", "" }, /* GLO */
...
*-----------------------------------------------------------------------------*/
static int satid2no(const char *id)
{
	int sys, prn;
	char code;
...
*-----------------------------------------------------------------------------*/
static int satexclude(int sat, double var, int svh, const prcopt_t *opt)
{
...
*-----------------------------------------------------------------------------*/
static int testsnr(int base, int idx, double el, double snr,
...
* return : carrier frequency (Hz) (0.0: error)
*-----------------------------------------------------------------------------*/
static double sat2freq(int sat, uint8_t code, const nav_t *nav)
{
	int i, fcn = 0, sys, prn;
...
* return : none
*-----------------------------------------------------------------------------*/
static void setcodepri(int sys, int idx, const char *pri)
{
...
*-----------------------------------------------------------------------------*/
static uint16_t rtk_crc16(const uint8_t *buff, int len)
{
	uint16_t crc = 0;
	int i;
...
*-----------------------------------------------------------------------------*/
static int decode_word(uint32_t word, uint8_t *data)
{
...
*-----------------------------------------------------------------------------*/
static int read_leaps(const char *file)
{
	FILE *fp;
	int i, n;
...
* get time string
* args   : gtime_t t        I   gtime_t struct
*          int    n         I   number of decimals(秒的小数位数)
* return : time string
...
* notes  : file with the staticsion .atx or .ATX is recognized as antex
*          file except for antex is recognized ngs antenna parameters
*          see reference [3]
*          only support non-azimuth-depedent parameters
*-----------------------------------------------------------------------------*/
static int readpcv(const char *file, pcvs_t *pcvs)
{
	pcv_t *pcv;
	char *ext;
	int i, j, stat;
...
*-----------------------------------------------------------------------------*/
static pcv_t *searchpcv(int sat, const char *type, gtime_t time,
	const pcvs_t *pcvs)
{
	pcv_t *pcv;
...
* return : none
*-----------------------------------------------------------------------------*/
static void readpos(const char *file, const char *rcv, double *pos)
{
...
* return : status (1:ok,0:file open error)
*-----------------------------------------------------------------------------*/
static int readerp(const char *file, erp_t *erp)
{
	FILE *fp;
	erpd_t *erp_data;
	double v[14] = { 0 };
...
*-----------------------------------------------------------------------------*/
static int readnav(const char *file, nav_t *nav)
{
	FILE *fp;
	eph_t eph0 = { 0 };
	geph_t geph0 = { 0 };
...
static int savenav(const char *file, const nav_t *nav)
{
	FILE *fp;
	int i;
	char id[32];
...
* return : none
*-----------------------------------------------------------------------------*/
static void freenav(nav_t *nav, int opt)
{
...
static void traceopen(const char *file) {}
static void traceclose(void) {}
static void tracelevel(int level) {}
static void trace(int level, const char *format, ...) {}
static void tracet(int level, const char *format, ...) {}
static void tracemat(int level, const double *A, int n, int m, int p, int q) {}
static void traceobs(int level, const obsd_t *obs, int n) {}
static void tracenav(int level, const nav_t *nav) {}
static void tracegnav(int level, const nav_t *nav) {}
static void tracehnav(int level, const nav_t *nav) {}
static void tracepeph(int level, const nav_t *nav) {}
static void tracepclk(int level, const nav_t *nav) {}
static void traceb(int level, const uint8_t *p, int n) {}
...
*-----------------------------------------------------------------------------*/
static int reppath(const char *path, char *rpath, gtime_t time, const char *rov,
	const char *base)
{
...
*-----------------------------------------------------------------------------*/
static int reppaths(const char *path, char *rpath[], int nmax, gtime_t ts,
...
*-----------------------------------------------------------------------------*/
static double geodist(const double *rs, const double *rr, double *e)
{
	double r;
	int i;
...
*-----------------------------------------------------------------------------*/
static double satazel(const double *pos, const double *e, double *azel)
{
...
static void dops(int ns, const double *azel, double elmin, double *dop)
{
...
*-----------------------------------------------------------------------------*/
static double tropmodel(gtime_t time, const double *pos, const double *azel,
	double humi)
{
...
*-----------------------------------------------------------------------------*/
static double tropmapf(gtime_t time, const double pos[], const double azel[],
	double *mapfw)
{
#ifdef IERS_MODEL
	const double ep[] = { 2000, 1, 1, 12, 0, 0 };
...
*-----------------------------------------------------------------------------*/
static void antmodel(const pcv_t *pcv, const double *del, const double *azel,
	int opt, double *dant)
{
...
* return : none
*-----------------------------------------------------------------------------*/
static void sunmoonpos(gtime_t tutc, const double *erpv, double *rsun,
...
{
	gtime_t time = { 0 };
	double ep[6] = { 2021, 5, 21, 0, 0, 0 };


	time = epoch2time(ep);


	return timeadd(time, 0);
}
...
#ifndef WIN_DLL
static int showmsg(char *format, ...) { return 0; }
                          ...
                          sigind_t *index, obsd_t *obs)
{
    ...
    for (i=0,j=ver<=2.99?0:3;i<ind->n;i++,j+=16) {
        ...
        if (stat) {
			...
			//if (lli[i] != 0)
			//{
			//	printf("\n%s\n %f %f", buff, str2num(buff, j, 14),str2num(buff, j + 14, 1));
			//	lli[0] = 0;
			//}
...
int ReadRinex(char **files, int n, obs_t *obs, nav_t *nav, int *nepoch)
{
	int i, stat;
	gtime_t ts = { 0 };
	gtime_t te = { 0 };
	...
	for (i = 0; i < n; i++)
	{
		stat = readrnxfile(files[i], ts, te, 0, &opt, 0, i + 1, &type, obs, nav, NULL);
	}
	if (obs->n <= 0)	return 0;
	if (nav->n <= 0)	return 0;
	*nepoch = sortobs(obs);
...
/* permutations --------------------------------------------------------------*/
static void perm(uint16_t n, double *L, double *D, uint16_t j, double del, double *Z)
{
	uint16_t k;
