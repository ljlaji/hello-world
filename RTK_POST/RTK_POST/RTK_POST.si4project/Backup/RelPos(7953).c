...
extern uint8_t PntPos(const T_PRCOPT *opt,const T_OBS *obs, const uint8_t n, const T_NAV *nav,T_SOL *sol,T_SSAT  *ssat)
{
	...
	if(ssat) /*save sat data*/
	{
		...
		for(i=0;i<n;i++)
		{
			ssat->sat[i] = obs[i].sat;
			memcpy(ssat->pos[i*COORDIM],rs[i*6],COORDIM*sizeof(float64_t));
			memcpy(ssat->vel[i*COORDIM],rs[i*6+3],COORDIM*sizeof(float64_t));
			ssat->ele[i] = azel[i*2+1];
			ssat->azi[i] = azel[i*2];
			...
			memcpy(ssat->los[COORDIM*i],los,COORDIM*sizeof(float64_t));
			ssat->dis[i] = dis;			
		}
	}
    return stat;
...
#if 0
extern UINT RtkPos(const T_PRCOPT *opt,const T_OBSS *obss,T_SOL *sol,T_SSAT  *ssat)
{
	uint8_t i,j;
	for(i=0;i<obss->nu;i++)
	{
		for(j= obss->nu;j<obss->nb;j++)
		{
			if(obss->data[i].sat == obss->data[j].sat)
				{
